...
extern "C" int imSoftIoc_registerRecordDeviceDriver(struct dbBase *pdbbase);
...
#define DBD_BASE "dbd" OSI_PATH_SEPARATOR "imSoftIoc.dbd"
#define EXIT_BASE "db" OSI_PATH_SEPARATOR "imSoftIocExit.db"
...
bool lazy_dbd_loaded = false;

// static epicsThreadOnceId exitLaterOnce = EPICS_THREAD_ONCE_INIT;
// static int exitLaterStatus;

static void epicsExit1(int status)
{
    printf("%s: entering\n", __func__);
    // would stop errlog and taskwd threads?
    // epicsExitCallAtExits();

    epicsThreadSleep(0.1);
    
    // OK since we use iocBuildIsolated()
    iocShutdown();

    // dbUnitTest.c testdbCleanup
...
static void epicsExitLater(int status)
{
...
static void usage(const char *arg0, const std::string& base_dbd) {
    std::cout<<"Usage: "<<arg0<<
               " [-D imSoftIoc.dbd] [-h] [-S] [-s] [-v] [-a ascf]\n"
               "[-m macro=value,macro2=value2] [-d file.db]\n"
               "[-x prefix] [st.cmd]\n"
               "\n"
               "    -D <dbd>  If used, must come first. Specify the path to the imSoftIoc.dbdfile."
               ...
               "        performed.\n"
               "\n"
               "    -x <prefix>  Load imSoftIocExit.db.  Provides a record \"<prefix>:exit\".\n"
               ...
               "\n"
               "Compiled-in path to imSoftIoc.dbd is:\n"
               "\t"<<base_dbd.c_str()<<"\n";
}

static void errIf(int ret, const std::string& msg)
{
    if(ret)
        throw std::runtime_error(msg);
}

static void lazy_dbd(const std::string& dbd_file) {
    printf("%s: entering %d\n", __func__, lazy_dbd_loaded);
    if(lazy_dbd_loaded) return;
    lazy_dbd_loaded = true;

    if (verbose)
        std::cout<<"iocshRegisterCommon()\n";
    iocshRegisterCommon();
    ...
    if (verbose)
        std::cout<<"imSoftIoc_registerRecordDeviceDriver(pdbbase)\n";
    errIf(imSoftIoc_registerRecordDeviceDriver(pdbbase),
          "Failed to initialize database");

    // TODO: use this to allow using a PV to exit the IOC?
    //       not really useful since we do not have a remote CA client
...
int ioc_main(int argc, char *argv[])
{
    try {
        ...
        // NOTE: reset this otherwise the getopt() does not work on subsequent
        //       invocations!!!!
        optind = 1;
        ...
        while ((opt = getopt(argc, argv, "ha:D:d:m:Ssx:v")) != -1) {
            ...
            case 'D':
                if(lazy_dbd_loaded) {
                    throw std::runtime_error("-D specified too late.  imSoftIoc.dbd already loaded.\n");
                }
                dbd_file = optarg;
                break;
            case 'd':
        ...
        if (loadedDb) {
            ...
            // NOTE: using this results in "unclean" IOC shutdown,
            //       aka threads remain running
            // iocInit();

            // see dbUnitTest.c
            // use the IOC without the CA server.. should be OK for
            // the purpose at hand ATM..
            int ret;
